...
export default {
    ...
    methods: {
        async submitForm() {
            ...
            this.errors.push("Username 3 ta belgidan ko'proq bolishi kerak!");
        }
            if (this.form.password === '') {
            this.errors.push('Iltimos Parolni kiriting');
        }
            if (this.form.password.length <= 6) {
            this.errors.push("Parol 6 ta belgidan ko'proq bolishi k/k");
        }
          ...
          <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
              ...
              <form v-on:submit.prevent="submitForm" class="space-y-4 md:space-y-6" action="#">
                  <div>
                      ...
                      <input v-model="form.username" type="username" name="username" id="username" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="username">
                  </div>
                  <div>
                  ...
                  <p class="text-sm font-light text-gray-500 dark:text-gray-400">
                      Create an account. 
                      <RouterLink  :to="{ name: 'signup' }" class="font-medium text-primary-600 hover:underline dark:text-primary-500">Sign Up</RouterLink>
                  </p>
              </form>
          </div>
      </div>
