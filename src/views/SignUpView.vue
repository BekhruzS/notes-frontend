    ...
    data() {
     ...
     methods: {
        async submitForm() {
            ...
            if (this.form.email === '') {
            this.errors.push('Iltimos Email kiriting');
        }
            if (this.form.password === '') {
            this.errors.push("Iltimos Parolni kiriting.");
        }
            if (this.form.password.length <= 8) {
            this.errors.push("Parol 8 ta belgidan iborat bolishi k/k.");
        }
                    ...
                    if (error.response && error.response.status === 400) {
                        if (error.response.data.errors && error.response.data.errors.username) {
                            this.errors.push("Ushbu username allaqachon ishlatilgan.");
                                ...
                                this.errors.push("Parol bir biriga mis kelmadi.");
                            } else {
                                console.error('Unkown error:', error.response.data.errors);
                        }
                    }
                }
            }
              ...
              <form v-on:submit.prevent="submitForm" class="space-y-4 md:space-y-6" action="#">
                <div>
                  ...
                  <div>
                      ...
                      <input v-model="form.password2" type="confirm-password" name="confirm-password" id="confirm-password" placeholder="••••••••" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                  </div>
                  ...
                  <p class="text-sm font-light text-gray-500 dark:text-gray-400">
                      Already have an account?                       <RouterLink  :to="{ name: 'login' }" class="font-medium text-primary-600 hover:underline dark:text-primary-500">Sign In</RouterLink>
                  </p>
              </form>
          </div>
      </div>
